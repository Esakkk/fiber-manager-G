...
function createStandaloneCustomerIcon(customer) {
    let color = '#718096'; // Tanpa ONT
    if (customer && customer.onu_number) {
        if (customer.acs_status === 'online') {
            color = '#48bb78';
        } else {
            color = '#e53e3e';
        }
    ...
    return L.divIcon({
        html: `
            <div style="position: relative; width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; background: white; border-radius: 50%; border: 2.5px solid ${color}; box-shadow: 0 2px 6px rgba(0,0,0,0.3); transition: transform 0.2s ease;">
                <i class="fas fa-home" style="color: ${color}; font-size: 13px;"></i>
            </div>
        `,
            ...
            odp.ports.forEach(port => {
                if (port.status === 'used' && port.lat && port.lng) {
                    ...
                    if (!isNaN(cLat) && !isNaN(cLng) && cLat >= -90 && cLat <= 90 && cLng >= -180 && cLng <= 180) {
                        ...
                        let customerColor = '#718096'; // Tanpa ONT
                        if (port.onu_number) {
                            if (port.acs_status === 'online') {
                                customerColor = '#48bb78';
                            } else {
                                customerColor = '#e53e3e';
                            }
                        }

                        const customerIcon = L.divIcon({
                            html: `<div style="position: relative; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; background: white; border-radius: 50%; border: 2px solid ${customerColor}; box-shadow: 0 2px 5px rgba(0,0,0,0.3);"><i class="fas fa-home" style="color: ${customerColor}; font-size: 12px;"></i></div>`,
                        ...
                        customerMarker.bindPopup(`
                            <div style="min-width: 200px;">
                                <h4 style="margin: 0 0 10px 0; color: ${customerColor};"><i class="fas fa-user"></i> ${port.target || 'Pelanggan'}</h4>
                                <p style="margin: 4px 0;"><strong>ODP:</strong> ${odp.name}</p>
                                <p style="margin: 4px 0;"><strong>Port:</strong> ${port.port_number}</p>
                                <p style="margin: 4px 0;"><strong>Status:</strong> <span style="color: ${customerColor}; font-weight: bold;">${port.onu_number ? (port.acs_status === 'online' ? '🟢 Online' : '🔴 Offline / LOS') : '✕ Tanpa ONT'}</span></p>
                        ...
                        if (isLinesEnabled) {
                            ...
                            line.originalStyle = { color: customerColor, weight: 2, opacity: 0.7, dashArray: '4, 4' };
                            line.lineType = 'drop';
    ...
    if (devices.customer) {
        devices.customer.forEach(customer => {
            if (!customer.odp_id) {
                ...
                if (!isNaN(cLat) && !isNaN(cLng) && cLat >= -90 && cLat <= 90 && cLng >= -180 && cLng <= 180) {
                    const marker = L.marker([cLat, cLng], { icon: createStandaloneCustomerIcon(customer) });
                    ...
                    let customerColor = '#718096';
                    if (customer.onu_number) {
                        customerColor = customer.acs_status === 'online' ? '#48bb78' : '#e53e3e';
                    }
                    
                    marker.bindPopup(`
                        <div style="min-width: 200px;">
                            <h4 style="margin: 0 0 10px 0; color: ${customerColor};"><i class="fas fa-user"></i> ${customer.name}</h4>
                            <p style="margin: 4px 0; color: #e53e3e; font-weight: bold;"><i class="fas fa-exclamation-circle"></i> Belum Terhubung ke ODP</p>
                            <p style="margin: 4px 0;"><strong>Status ONT:</strong> <span style="color: ${customerColor}; font-weight: bold;">${customer.onu_number ? (customer.acs_status === 'online' ? '🟢 Online' : '🔴 Offline / LOS') : '✕ Tanpa ONT'}</span></p>
...
function showCustomerInfo(customerData) {
    ...
    if (port.onu_number) {
        if (port.acs_status === 'online') {
            statusHtml = '<span style="color: #48bb78; font-weight: bold;">🟢 Online (GenieACS)</span>';
        } else {
            statusHtml = '<span style="color: #e53e3e; font-weight: bold;">🔴 LOS / Offline (GenieACS)</span>';
        }
    } else {
        statusHtml = '<span style="color: #718096; font-weight: bold;">✕ Tanpa ONT (Kosong)</span>';
    }
    ...
    let html = `<div class="device-detail">
        ...
        <p style="margin: 4px 0;"><strong>Status:</strong> ${statusHtml}</p>
        ${port.onu_number ? `
            <p style="margin: 4px 0;"><strong>IP Address:</strong> ${port.acs_ip || '-'}</p>
            <p style="margin: 4px 0;"><strong>Last Inform:</strong> ${port.acs_last_inform || '-'}</p>
            <p style="margin: 4px 0;"><strong>Optical Power (RX):</strong> ${port.acs_rx_power ? `${port.acs_rx_power} dBm` : '-'}</p>
        ` : ''}
        <hr>
...
function showStandaloneCustomerInfo(customer) {
    ...
    let html = `<div class="device-detail">
        ...
        <p style="margin: 4px 0;"><strong>Koneksi ODP:</strong> <span style="color: #718096; font-weight: bold;">✕ Belum Terhubung</span></p>
        <p style="margin: 4px 0;"><strong>Status ONT:</strong> ${statusHtml}</p>
        ${customer.onu_number ? `
            <p style="margin: 4px 0;"><strong>IP Address:</strong> ${customer.acs_ip || '-'}</p>
            <p style="margin: 4px 0;"><strong>Last Inform:</strong> ${customer.acs_last_inform || '-'}</p>
            <p style="margin: 4px 0;"><strong>Optical Power (RX):</strong> ${customer.acs_rx_power ? `${customer.acs_rx_power} dBm` : '-'}</p>
        ` : ''}
        <hr>
        <h4 style="margin: 10px 0 8px 0;"><i class="fas fa-map-pin"></i> Informasi Lokasi</h4>
        <p style="margin: 4px 0;"><strong>Koordinat Pelanggan:</strong></p>
