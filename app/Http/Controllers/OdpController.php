...
class OdpController extends Controller
{
    ...
    protected function getAllODP()
    {
        ...
        ])->orderBy('created_at', 'desc')->get()->map(function ($odp) use ($acsService) {
            $odp->source_name = $odp->source ? $odp->source->name : null;

            $ports = $odp->ports->map(function ($port) use ($acsService) {
                if ($port->status === 'used' && $port->onu_number) {
                    $acsData = $acsService->getAcsData($port->onu_number);
                    $port->acs_status = $acsData['acs_status'];
                    $port->acs_last_inform = $acsData['acs_last_inform'];
                    $port->acs_ip = $acsData['acs_ip'];
                    $port->acs_rx_power = $acsData['acs_rx_power'];
                } else {
                    $port->acs_status = 'offline';
                    $port->acs_last_inform = null;
                    $port->acs_ip = null;
                    $port->acs_rx_power = null;
                }
                return $port;
            });
            $odp->ports = $ports;
    ...
    protected function getODP($id)
    {
        ...
        if ($odp) {
            ...
            $acsService = app(GenieAcsService::class);
            $ports = $odp->ports()->orderBy('port_number')->get()->map(function ($port) use ($acsService) {
                if ($port->status === 'used' && $port->onu_number) {
                    $acsData = $acsService->getAcsData($port->onu_number);
                    $port->acs_status = $acsData['acs_status'];
                    $port->acs_last_inform = $acsData['acs_last_inform'];
