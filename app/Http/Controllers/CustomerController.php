...
class CustomerController extends Controller
{
    ...
    protected function getAllCustomers(Request $request)
    {
        ...
        $acsService = app(GenieAcsService::class);
        $customers = $query->get()->map(function ($c) use ($acsService) {
            $c->odp_name = $c->odp ? $c->odp->name : null;
            if ($c->onu_number) {
                $acsData = $acsService->getAcsData($c->onu_number);
                $c->acs_status = $acsData['acs_status'];
                $c->acs_last_inform = $acsData['acs_last_inform'];
                $c->acs_ip = $acsData['acs_ip'];
                $c->acs_rx_power = $acsData['acs_rx_power'];
            } else {
                $c->acs_status = 'offline';
                $c->acs_last_inform = null;
                $c->acs_ip = null;
                $c->acs_rx_power = null;
            }
            return $c;
    ...
    protected function getCustomer($id)
    {
        ...
        if ($customer) {
            ...
            if ($customer->onu_number) {
                $acsData = $acsService->getAcsData($customer->onu_number);
                $customer->acs_status = $acsData['acs_status'];
                $customer->acs_last_inform = $acsData['acs_last_inform'];
                $customer->acs_ip = $acsData['acs_ip'];
                $customer->acs_rx_power = $acsData['acs_rx_power'];
            } else {
                $customer->acs_status = 'offline';
                $customer->acs_last_inform = null;
                $customer->acs_ip = null;
                $customer->acs_rx_power = null;
            }
